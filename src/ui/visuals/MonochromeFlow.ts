...
export class MonochromeFlow {
    ...
    public update(dt: number, audioData: { low: number, high: number, kick: boolean, spectrum?: any }) {
        ...
        const isBlueFlash = levels[4] > 0.4; 
        const isRedFlash = levels[0] > 0.6; // Deep bass rumble
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            // Default 0.01 -> 0.03
            const turbulence = 0.01 + (levels[0] * 0.03);
            ...
            const tz = z * s;
            const vx = Math.sin(ty) - Math.cos(tz);
            const vy = Math.sin(tz) - Math.cos(tx); // Fix typo in previous?
            const vz = Math.sin(tx) - Math.cos(ty);
            ...
            if (meta.band === 0) {
                ...
                // Huge multiplier: 1x -> 6x
                speed *= (1.0 + bandLevel * 5.0);
            } else if (meta.band === 4) {
                // High: Jittery
                speed *= (1.0 + bandLevel * 2.0);
                // Jitter
                if (bandLevel > 0.1) {
                    jitter = bandLevel * 0.8; 
                }
            } else {
                // Mids: Standard flow acceleration
                speed *= (1.0 + bandLevel * 3.0);
            }
