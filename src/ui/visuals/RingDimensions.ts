...
export class RingDimensions {
            ...
            const material = new THREE.ShaderMaterial({
                ...
                fragmentShader: `
                    ...
                    void main() {
                        ...
                        float edge = 0.02; // Sharper (was 0.1)
                        
                        float alpha = smoothstep(threshold, threshold + edge, vVol);
                        
                        float rim = abs(vUv.y - 0.5) * 2.0; 
                        alpha *= smoothstep(1.0, 0.6, rim); 

                        float timeFade = min(smoothstep(0.0, 0.05, vUv.x), smoothstep(1.0, 0.95, vUv.x));
                        alpha *= timeFade;
                        
                        vec3 col = uColor;
                        gl_FragColor = vec4(col, alpha * 0.95 * uGlobalOpacity);
                    }
                `,
                transparent: true,
    ...
    public update(dt: number, audioData: { low: number, high: number, kick: boolean, spectrum?: any, crossfader?: number }) {
        ...
        this.ringMeshes.forEach((mesh, i) => {
            ...
            let speed = baseSpeed + (vol * 2.0 * dir);
        ...
        if (this.cilia) {
            this.cilia.rotation.z += 0.005 * dt;
            
            const mat = this.cilia.material as THREE.ShaderMaterial;
            if (mat.uniforms) {
                mat.uniforms.uLow.value = currentLevels[0];
                mat.uniforms.uHigh.value = currentLevels[4];
            }
        }
        
        if (this.particulates) {
             this.particulates.rotation.z += 0.02 * dt;
        }
    }
